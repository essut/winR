...
## Load all functions by running everything from line 4-133
...
sort.dlong <- function(dlong) {
  sorted.dlong <-
    dlong[
      do.call(
        order,
        dlong[, c("chromosome", "start.pos", "end.pos", "allele")]
      ),
    ]

  locus.order <- unique(sorted.dlong[["locus"]])
  sorted.dlong[["locus"]] <- factor(
    sorted.dlong[["locus"]],
    levels = locus.order
  )

  marker.order <- unique(sorted.dlong[["marker"]])
  sorted.dlong[["marker"]] <- factor(
    sorted.dlong[["marker"]],
    levels = marker.order
  )
...
plot.rainbow <- function(
  dlong.wprop,
  sample_id,
  add.marker.label = TRUE,
  sort.by.prop = FALSE
) {
